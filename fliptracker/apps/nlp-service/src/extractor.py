...
class HybridExtractor:
    def __init__(self):
        ...
        try:
            self.nlp_fr = spacy.load("fr_core_news_lg", disable=["parser", "lemmatizer"])
            self.nlp_en = spacy.load("en_core_web_lg", disable=["parser", "lemmatizer"])
        except OSError:
            print("⚠️ Models not found. Did you run 'python -m spacy download ...' in Dockerfile?")
            raise
    ...
    def process(self, raw_body: str, subject: str = "", sender: str = "") -> dict:
        ...
        for ent in doc.ents:
            if ent.label_ in ["ORG", "GPE", "DATE", "MONEY"]:
                ...
                if ent.label_ == "ORG" and not carrier:
                    if any(x in ent.text.lower() for x in ["poste", "colis", "relay", "ups", "dhl", "chronopost"]):
        ...
        return {
            ...
            "language": lang,
            "raw_text_snippet": clean_text[:100] # Pour vérifier le nettoyage
        }